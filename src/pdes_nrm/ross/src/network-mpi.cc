...
tw_net_init(int *argc, char ***argv)
{
  ...
  if(0) {
    printf("In %s: I am mpi rank %d. g_tw_mynode = %d\n",
	   __func__,my_rank,(int) g_tw_mynode);
  }
...
tw_net_minimum(tw_pe *me)
{
  ...
  for (i = 0; i < (int) posted_sends.cur; i++) {
...
recv_finish(tw_pe *me, tw_event *e, char * buffer)
{
  ...
  if(e->recv_ts < me->GVT)
    tw_error(TW_LOC, "%d: Received straggler from %d: %lf (%d)", 
	     me->id,  e->send_pe, e->recv_ts.real(), e->state.cancel_q);
