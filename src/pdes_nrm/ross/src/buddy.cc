...
int dump_buddy_table(buddy_list_bucket_t *buddy_master)
{
    ...
    while (1) {
        ...
        printf("\n");
        assert(counter == (int) blbt->count && "Count is incorrect!");
...
void buddy_free(void *ptr)
{
  buddy_list_t *blt = (buddy_list_t *) ptr;
    ...
    if (blbt->count == 0) {
        ...
        assert((int) blbt->count == initial_count + 1);
        return;
    }

    if (buddy_try_merge(blt)) {
      assert(initial_count > (int) blbt->count);
        return;
    }
...
buddy_list_bucket_t * create_buddy_table(unsigned int power_of_two)
{
    ...
    bsystem = (buddy_list_bucket_t *)
      tw_calloc(TW_LOC, "buddy system", list_count + 1, sizeof(buddy_list_bucket_t));
    if (bsystem == NULL) {
        return NULL;
    }
