...
void map_linear(void) {
    ...
    for(kpid = 0, lpid = 0, pe = NULL; (pe = tw_pe_next(pe)); ) {
      ...
      printf("\tPE %d\n", (int) pe->id);
        ...
        for(i = 0; i < (int) nkp_per_pe; i++, kpid++) {
            tw_kp_onpe(kpid, pe);

#if VERIFY_MAPPING
            printf("\t\tKP %d", (int) kpid);
#endif

            for(j = 0;
		j < (int) nlp_per_kp && lpid < (int) g_tw_nlp;
		j++, lpid++) {
                ...
                printf("%lld (%lld) ",
		       (long long int) (lpid+g_tw_lp_offset),
		       (long long int) (g_tw_lp[lpid]->id));
#endif
            }
    ...
    if(!g_tw_lp[g_tw_nlp-1]) {
      tw_error(TW_LOC, "Not all LPs defined! (g_tw_nlp=%d)", (int) g_tw_nlp);
    }
...
#if VERIFY_MAPPING
        printf("LP %4d (from LP array %d) KP %4d PE %4d\n",
	       i,
	       (int) g_tw_lp[i]->id,
	       (int) g_tw_kp[kpid]->id,
	       (int) pe->id);
...
static void tw_delta_alloc(tw_pe *pe) {
    ...
    pe->delta_buffer[0] = (unsigned char *)
      tw_calloc(TW_LOC, "Delta buffers", g_tw_delta_sz, 1);
    pe->delta_buffer[1] = (unsigned char *)
      tw_calloc(TW_LOC, "Delta buffers", g_tw_delta_sz, 1);
}
