...
void tw_event_send(tw_event * event) {
    ...
    if (g_tw_synchronization_protocol == CONSERVATIVE) {
      if (recv_ts.real() - tw_now(src_lp).real() >= g_tw_lookahead) {
...
static inline void event_cancel(tw_event * event) {
    ...
    } else if (send_pe->node == (int) dest_peid) {
...
void tw_event_cancel(tw_event *evt) {
  /* Only call this function of you know what you are doing.
     An event cancelled this way can not be uncancelled, and
     hence you must guarantee that this cancellation will not
     be rolled back. */
  event_cancel(evt);
}
