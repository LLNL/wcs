...
#else
	union { struct tw_bf bf; uint32_t u; } zero;
	zero.u = 0;
	revent->cv = zero.bf;
#endif
    }
...
#else
	union { struct tw_bf bf; uint64_t u; } zero;
	zero.u = 0;
	revent->cv = zero.bf;
#endif
...
void tw_scheduler_sequential(tw_pe * me) {
    ...
    while ((cev = tw_pq_dequeue(me->pq))) {
        ...
        gvt = cev->recv_ts;
        if(gvt.real() / g_tw_ts_end > percent_complete &&
	   (g_tw_mynode == g_tw_masternode)) {
            gvt_print(gvt);
        }
	...
	/* Call commit function */ {
	  if(clp->type->commit != NULL)
	    (*clp->type->commit)(clp->cur_state, &cev->cv,
        ...
        */
        for (msg_i = g_tw_mblock; msg_i; msg_i--) {
            ...
            */
            ...
            if(tw_pq_minimum(me->pq).real() >=
	       me->GVT.real() + g_tw_lookahead) {
                break;
            }
            ...
            if( ckp->last_time > cev->recv_ts ){
			 ...
			 ckp->last_time.real(), cev->recv_ts.real(), clp->gid );
            }
            ckp->last_time = cev->recv_ts;
